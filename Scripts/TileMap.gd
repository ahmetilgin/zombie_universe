...
func left_and_right_area_control(points_relative):
	...
	var left_has_tile = get_cellv(points_relative[0])  != INVALID_CELL
	var right_has_tile = get_cellv(points_relative[1]) != INVALID_CELL
	var top_has_tile = get_cellv(points_relative[2]) != INVALID_CELL
	var bottom_has_tile = get_cellv(points_relative[3]) != INVALID_CELL
