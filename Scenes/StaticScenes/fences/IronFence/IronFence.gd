...
func _ready():
	$IronFenceFront.play("idle")
	$IronFenceBack.play("idle")
...
func flash_damage():
	for i in range(N_FLASHES * 2):
		var color = $IronFenceBack.modulate if i % 2 == 1 else  flash_color
		var color2 = $IronFenceFront.modulate if i % 2 == 1 else  flash_color
		var time = FLASH_RATE * i +FLASH_RATE
		flash_fence_tween.interpolate_callback($IronFenceBack,time, "set", "modulate", color)
		flash_fence_tween.interpolate_callback($IronFenceFront,time, "set", "modulate", color2)
